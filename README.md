...
`script` on `/screenshot` is a pre-capture interaction: it runs against the live page after navigation (with the tab foregrounded, so `requestAnimationFrame` is active) and before `wait_for`/`settle_ms`/capture, then the same page is screenshotted. This captures views that only exist after a client-side interaction (an expanded node, an applied filter) which a fresh navigation cannot reach. The script's return value comes back as `script_result` (omitted if no script ran), so one call yields both the image and assertion data — e.g. a post-interaction node count. On the CLI it is `--script JS` or `--script-file PATH` (the file form avoids shell-quoting multi-line scripts); `script_result` is printed to stderr so stdout stays a clean PNG stream. Promises are awaited, so the script can `await` the app's readiness before acting. See [docs/authenticated-captures.md](./docs/authenticated-captures.md) for the interact-then-capture recipe.
